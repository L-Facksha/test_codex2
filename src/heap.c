...
/*   Updated: 2026/08/07 17:39:55 by azebahad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	heap_push(t_heap *heap, t_request req, char *scheduler)
{
	...
	{
		parent = (current - 1) / 2;
		if (!request_has_higher_priority(&heap->data[current],
...
void	heap_pop(t_heap *heap, char *scheduler)
{
	...
	while (left_child < heap->size)
	{
		...
		if (!request_has_higher_priority(&heap->data[best_child],
				&heap->data[current], scheduler))
			break ;
		heap_swap(&heap->data[current], &heap->data[best_child]);
		current = best_child;
		left_child = (current * 2) + 1;
	}
}
